...
void ReadRecordFight::showGameOver(std::string winner) {
    ...
    this->gameOverLabel->setSystemFontSize(10);
...
//    rapidjson::Value& roundArray = dataArray[0]["rounds"];
    ...
    if (this->currentBigIndex >= roundArray.Size()) {
        if (dataArray.IsArray() == true) {
            ...
            if (dataArray.HasMember("result")) {
                rapidjson::Value& result = dataArray["result"];
                bool isover = result["isgameover"].GetBool();
                if (isover == true) {
                    ...
                    auto dispatcher = Director::getInstance()->getEventDispatcher();
                    EventCustom event = EventCustom("pveGameOver");
                    dispatcher->dispatchEvent(&event);
                }
            }
        }
        ...
        if(this->fightName.compare("pve") == 0) {
            auto dispatcher = Director::getInstance()->getEventDispatcher();
            EventCustom event = EventCustom("pveRoundNext");
            dispatcher->dispatchEvent(&event);
        }
