...
Label* CommonFunc::createTextInfoLabel(std::string text, cocos2d::Vec2 position) {
    auto label = Label::createWithTTF(text, "fonts/fangzhengjingheijianti.ttf", 16);
    label->setDimensions(80, 20);
    label->enableBold();
    label->setTextColor(tuJianTextColor);
...
Label* CommonFunc::createLabel(std::string text, cocos2d::Vec2 position) {
    auto label = Label::createWithTTF(text, "fonts/kaiti.ttf", 16);
    label->setDimensions(80, 20);

    label->setTextColor(tuJianInfoColor);
    label->setAlignment(TextHAlignment::CENTER);
    label->setPosition(position);
    return label;
}
